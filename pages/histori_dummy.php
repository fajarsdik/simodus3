...
if (empty($_SESSION['admin'])) {
    ...
    <div id = "page-wrapper">
        <div class = "row">
            <div  class = "col-lg-12">
                <h1 class = "page-header">History Pemakaian Dummy No. <?= $dummy ?></h1>
            </div>
        </div>
        <!-- Akhir Judul -->
                    ...
                    <div class="table-responsive">
                        <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-1">
                            ...
                            <tbody>
                                ...
                                $query = mysqli_query($config, "SELECT * FROM tbl_metdum_pakai WHERE unit LIKE '$unit%' && aktivasi ='non aktif' ORDER by tgl_pakai ASC LIMIT $curr, $limit");
                                if (mysqli_num_rows($query) > 0) {
                                    $no = 1;
                                    while ($row = mysqli_fetch_array($query)) {
                                        echo ' 
